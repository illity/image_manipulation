...
sz, scale, p, q = 80, 10, 0.4, 0.4#chance em x, chance em y
...
writer = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc(*'xvid'), 20,(800,800))
...
for i in range(1,int(np.amax(path))+1):
    for pos in np.reshape(np.where(path==i),(2,-1)).T:
        ...
        cv2.fillPoly(img,sqr,(100+155*(path[pos[0],pos[1]]/np.amax(path)),0,0))
    writer.write(img)
...
for i in range(len(shortcut)-1,-1,-1):
    sqr = np.array([[[scale*shortcut[i][1]+1,scale*shortcut[i][0]+1],[scale*(shortcut[i][1]+1)-1,scale*shortcut[i][0]+1],[scale*(shortcut[i][1]+1)-1,scale*(shortcut[i][0]+1)-1],[scale*shortcut[i][1]+1,scale*(shortcut[i][0]+1)-1]]]).astype(np.int32);
    cv2.fillPoly(img,sqr,(0,0,100+155*path[shortcut[i][0]][shortcut[i][1]]/np.amax(path)))
    writer.write(img)
    cv2.imshow('',img); cv2.waitKey()
